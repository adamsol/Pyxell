        ...
        if attr == 'toString' and type.isPrintable():
            ...
            if attr == 'length':
                ...
                value = {
                    ...
                    'pop': v.Variable(t.Func([type], t.Tuple([type.key_type, type.value_type])), attr),
    ...
    def transpileStmtFor(self, node):
        ...
        for iterable in iterables:
            ...
            if iterable['node'] == 'ExprRange':
                ...
                if type.isGenerator():
                    next = v.Call(v.Attribute(value, 'next'), type=type.subtype)
                    if type.subtype == t.Void:
                        self.output(next)
                        getter = self.var(t.Void)
                    else:
                        getter = self.tmp(next)
                else:
